...
public:
	...
	virtual std::string address_name_from_stream( std::istream& st, unsigned int& userDataOctetLenght, unsigned int userDataHeaderOctetLength = 0 )
	{
		std::stringstream/*<typename Base::char_type, typename Base::traits_type> */ ss;
		unsigned int n = 0;
		unsigned char cx = 0;
			 ...
			 && (userDataHeaderOctetLength + 1) % 7)
		{
			unsigned char x = (unsigned char)st.get();
			if (!st)
				throw typename std::exception( "Default User Data: input stream read failure" );
			--userDataOctetLenght;
    ...
    void from_stream(std::istream& st)
    {
        ...
        for (unsigned char i = 0; i < len + 1; ++i)
        {
            ...
            else if (1 == i)
            {
				...
				{
					uint nLen = len - 1;
					std::string sAddress = address_name_from_stream( st, nLen );
...
enum class EOriginatorAddressType
{
...
public:
	...
	EOriginatorAddressType originatorAddressType() const { return EOriginatorAddressType( address_.addressType() ); }
